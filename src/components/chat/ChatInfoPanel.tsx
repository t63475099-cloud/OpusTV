...
export default function ChatInfoPanel({
  ...
  const promoteAdmin = useChatStore((s) => s.promoteAdmin);
  const demoteAdmin = useChatStore((s) => s.demoteAdmin);
  const isGroupAdmin = useChatStore((s) => s.isGroupAdmin);
  ...
  const members = conversation.participants
    .map((id) => getUser(id) || { id, name: id, nickname: id, avatar: "", status: "offline" as const })
    .filter(Boolean);
  const [ann, setAnn] = useState(conversation.announcement || "");
  const [gTitle, setGTitle] = useState(conversation.title || "");
  const [addId, setAddId] = useState("");

  const iAmAdmin = conversation.isGroup && isGroupAdmin(conversation.id, me);
      ...
      <div className="flex-1 overflow-y-auto opus-chat-scroll custom-scroll p-4 space-y-5">
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          >
            ...
            <span className="text-[11px]">
              {conversation.muted ? "Bật TB" : "Tắt TB"}
            </span>
          </button>
        </div>
        ...
        <div className="space-y-2">
          ...
          <div className="flex flex-wrap gap-1.5">
            {[
              { h: 1 as number | null, label: "1 giờ" },
              { h: 8, label: "8 giờ" },
              { h: 24, label: "1 ngày" },
              { h: null, label: "Bật lại" },
            ].map((o) => (
              <button
                key={String(o.label)}
                type="button"
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className="space-y-2">
                {members.map((u) => {
                  if (!u) return null;
                  const admin = isGroupAdmin(conversation.id, u.id);
                  return (
                        ...
                        <div className="flex flex-col gap-0.5">
                          {!admin ? (
                            <button
                              ...
                              onClick={() =>
                                promoteAdmin(conversation.id, u.id)
                              }
                            >
                              Thêm admin
                            </button>
                            ...
                            <button
                              type="button"
                              className="text-[10px] text-zinc-400 hover:text-zinc-300 px-1"
                              onClick={() => demoteAdmin(conversation.id, u.id)}
                            >
                              Gỡ admin
                            </button>
                          )}
                          <button
                            ...
                            onClick={() => {
                              if (
                                window.confirm(
                                  `Xóa ${u.name || u.id} khỏi nhóm?`
                                )
                              ) {
                                removeGroupMember(conversation.id, u.id);
                              }
                            }}
                          >
                            Xóa
            ...
            <button
              type="button"
              className="w-full flex items-center justify-center gap-2 rounded-xl py-2.5 text-sm font-medium bg-rose-600/15 text-rose-300 hover:bg-rose-600/25"
              onClick={() => {
                if (window.confirm("Bạn chắc chắn muốn rời nhóm này?")) {
